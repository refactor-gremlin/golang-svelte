...
/**
 * Safe wrapper for validateFormData that handles experimental remote function type casting.
 *
 * WHY THIS FUNCTION:
 * - Hides the ugly `as unknown as ExperimentalFormData` casting from business logic
 * - Provides a clean API for all form handlers
 * - Centralizes the experimental function workaround in one place
 * - Easy to remove when SvelteKit stabilizes the experimental APIs
 *
 * @param formData - The formData object from SvelteKit's experimental form function (type: unknown)
 * @param schema - Zod schema for validation rules
 * @returns Validated and typed data ready for use
 * @throws 400 error with specific validation message if validation fails
 *
 ...
 *   // Clean API - no ugly casting in business logic
 *   const { username, email, password } = safeValidateFormData(formData, zRegisterFormWithConfirm);
 *   // Rest of your logic here
 * });
 * ```
 */
export function safeValidateFormData<T extends z.ZodType>(
	formData: unknown,
	schema: T
): z.infer<T> {
	return validateFormData(formData as unknown as ExperimentalFormData, schema);
}
